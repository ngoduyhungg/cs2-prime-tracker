...
type ActivePanel = "inventory" | "progress" | null;
...
function DashboardWorkspace({
    ...
    const showProgress = activePanel === "progress";
    ...
    const handleToggleInventory = () => {
        setActivePanel((prevPanel) =>
            prevPanel === "inventory" ? null : "inventory"
        );
    };

    const handleToggleProgress = () => {
        setActivePanel((prevPanel) =>
            prevPanel === "progress" ? null : "progress"
        );
    };
    ...
    return (
        <section className="grid gap-8 xl:grid-cols-[minmax(0,1fr)_420px] xl:items-start">
            <div className="min-w-0 space-y-6">
                <HeroBanner stats={stats} items={items}/>
                <StatsCards stats={stats} />

                {showInventory && <InventoryPanel items={items} />}
                        ...
                        {weeks.length === 0 ? (
                            <div className="rounded-3xl border border-dashed border-slate-700 bg-slate-900/60 px-6 py-12 text-center">
                                ...
                                <p className="mt-2 text-sm text-slate-400">
                                    Tạo tuần mới để bắt đầu theo dõi tiến trình
                                    hoàn vốn.
                                </p>
                            </div>
                            ...
                            <div className="rounded-3xl border border-dashed border-slate-700 bg-slate-900/60 px-6 py-12 text-center">
                                ...
                                <p className="mt-2 text-sm text-slate-400">
                                    Thử tìm bằng tên item, loại item hoặc ngày
                                    nhận khác.
                                </p>
            ...
            <aside className="space-y-5 xl:sticky xl:top-8">
                <BestDropCard items={items} compact />

                <BestSaleCard items={items} compact />

                <section className="rgb-border-card rgb-border-card-soft rounded-3xl border border-slate-800 bg-slate-900/80 p-5 shadow-sm">
                    <p className="text-xs font-semibold uppercase tracking-[0.2em] text-cyan-300">
                        Actions
                    </p>
